...
public class BusinessParamFilter implements Filter {
    ...
    public Result invoke(Invoker<?> invoker, Invocation invocation) throws RpcException {
        String ucId = RpcContext.getContext().getAttachment(com.lianjia.dubbo.gray.filter.Constants.FILTER_PARAM_UCID);
        if (StringUtils.isNotEmpty(ucId)) {
            // 从RpcContext里获取ucId并保存
            BusinessParamUtils.setUcId(ucId);
        } else {
            // 交互前重新设置ucId, 避免信息丢失
            RpcContext.getContext().setAttachment(com.lianjia.dubbo.gray.filter.Constants.FILTER_PARAM_UCID,
